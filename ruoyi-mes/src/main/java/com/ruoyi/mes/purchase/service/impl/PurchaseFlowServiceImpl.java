...
public class PurchaseFlowServiceImpl implements IPurchaseFlowService {
    ...
    public void approveOrder(Long id, String operator) {
        ...
        if (flowMapper.updateOrderStatus(id, DRAFT, APPROVED, operator) != 1)
            throw new ServiceException("采购订单状态已变化，请刷新后重试");
    }
    ...
    public void unapproveOrder(Long id, String operator) {
        ...
        if (flowMapper.countApprovedReceiptByOrder(id) > 0)
            throw new ServiceException("采购订单已生成已审核送货单，不能弃审");
        if (flowMapper.updateOrderStatus(id, APPROVED, DRAFT, operator) != 1)
            throw new ServiceException("采购订单状态已变化，请刷新后重试");
    }
    ...
    public void approveReceipt(Long id, String operator) {
        ...
        if (lines.isEmpty())
            throw new ServiceException("送货单至少需要一条明细");
        if (flowMapper.countReceiptSourceSuppliers(id) > 1)
            throw new ServiceException("一张送货单只能参照同一供应商的采购订单明细");
        ...
        if (flowMapper.updateReceiptStatus(id, DRAFT, APPROVED, operator) != 1)
            throw new ServiceException("送货单状态已变化，请刷新后重试");
    }
    ...
    public void unapproveReceipt(Long id, String operator) {
        ...
        if (!PENDING.equals(receipt.getInspectionStatus()))
            throw new ServiceException("送货单已完成质检，请先反质检后再弃审");
        if (flowMapper.countApprovedInboundByReceipt(id) > 0)
            throw new ServiceException("送货单已生成已审核入库单，不能弃审");
        for (PurchaseReceiptLine line : receiptMapper.selectLines(id)) {
            if (flowMapper.increaseOrderReceived(line.getSourceOrderLineId(), line.getReceivedQuantity().negate()) != 1)
                throw new ServiceException("订单到货数量反写失败");
        }
        if (flowMapper.updateReceiptStatus(id, APPROVED, DRAFT, operator) != 1)
            throw new ServiceException("送货单状态已变化，请刷新后重试");
    }
    ...
    public void inspectReceipt(Long id, InspectionRequest request, String operator) {
        ...
        if (!PENDING.equals(receipt.getInspectionStatus()))
            throw new ServiceException("送货单当前不是待检状态");
        List<PurchaseReceiptLine> lines = receiptMapper.selectLines(id);
        Map<Long, InspectionLineRequest> results = new HashMap<>();
        for (InspectionLineRequest result : request.getLines()) {
            if (results.put(result.getReceiptLineId(), result) != null)
                throw new ServiceException("质检明细不能重复");
        }
        if (results.size() != lines.size())
            throw new ServiceException("请填写全部送货明细的质检结果");
        ...
        for (PurchaseReceiptLine line : lines) {
            ...
            if (result == null || result.getQualifiedQuantity().add(result.getRejectedQuantity())
                    .compareTo(line.getReceivedQuantity()) != 0) {
            ...
            if (flowMapper.updateReceiptInspection(line) != 1 || flowMapper
                    .increaseOrderQualified(line.getSourceOrderLineId(), result.getQualifiedQuantity()) != 1) {
    ...
    public void uninspectReceipt(Long id, String operator) {
        ...
        if (PENDING.equals(receipt.getInspectionStatus()))
            throw new ServiceException("送货单尚未质检");
        if (flowMapper.countApprovedInboundByReceipt(id) > 0)
            throw new ServiceException("送货单已生成已审核入库单，不能反质检");
        for (PurchaseReceiptLine line : receiptMapper.selectLines(id)) {
            if (flowMapper.increaseOrderQualified(line.getSourceOrderLineId(),
                    line.getQualifiedQuantity().negate()) != 1)
                throw new ServiceException("订单合格数量反写失败");
        }
        flowMapper.resetReceiptInspection(id);
    ...
    public void approveInbound(Long id, String operator) {
        ...
        if (lines.isEmpty())
            throw new ServiceException("入库单至少需要一条明细");
        if (flowMapper.updateInboundStatus(id, DRAFT, APPROVED, operator) != 1)
            throw new ServiceException("入库单状态已变化，请刷新后重试");
        for (PurchaseInboundLine line : lines) {
            if (flowMapper.countValidInboundSource(line) != 1)
                throw new ServiceException("物料 " + line.getMaterialCode() + " 的来源送货明细无效");
            if (flowMapper.increaseReceiptInbound(line.getSourceReceiptLineId(), line.getInboundQuantity()) != 1
                    || flowMapper.increaseOrderInbound(line.getSourceOrderLineId(), line.getInboundQuantity()) != 1) {
                throw new ServiceException("物料 " + line.getMaterialCode() + " 的可入库数量不足，或来源送货单未完成质检");
            }
            InventoryTransaction transaction = buildTransaction(inbound, line, line.getInboundQuantity(),
                    InventoryTransactionType.INBOUND.getCode(), operator);
    ...
    public void unapproveInbound(Long id, String operator) {
        ...
        for (PurchaseInboundLine line : inboundMapper.selectLines(id)) {
            if (flowMapper.increaseReceiptInbound(line.getSourceReceiptLineId(),
                    line.getInboundQuantity().negate()) != 1
                    || flowMapper.increaseOrderInbound(line.getSourceOrderLineId(),
                            line.getInboundQuantity().negate()) != 1) {
                throw new ServiceException("物料 " + line.getMaterialCode() + " 的入库数量反写失败");
            }
            InventoryTransaction transaction = buildTransaction(inbound, line, line.getInboundQuantity(),
                    InventoryTransactionType.INBOUND_REVERSE.getCode(), operator);
            if (flowMapper.decreaseInventoryBalance(transaction) != 1)
                throw new ServiceException("物料 " + line.getMaterialCode() + " 的库存余额不足，不能弃审");
    ...
    /** 查询可参照生成到货单的订单明细。 */
    @Override
    public List<ReceiptReferenceLine> selectReceiptReferenceLines(String orderCode, String supplierName,
            String materialCode) {
    ...
    /** 查询可参照生成入库单的已质检到货明细。 */
    @Override
    public List<InboundReferenceLine> selectInboundReferenceLines(String receiptCode, String warehouseCode,
            String materialCode) {
        return flowMapper.selectInboundReferenceLines(receiptCode, warehouseCode, materialCode);
    }

    private PurchaseOrder requiredOrder(Long id) {
    ...
    private PurchaseInbound requiredInbound(Long id) {
        PurchaseInbound inbound = inboundMapper.selectById(id);
        if (inbound == null)
            throw new ServiceException("入库单不存在");
        return inbound;
    }
    private void assertStatus(String actual, String expected, String message) {
    ...
    private String resolveInspectionStatus(BigDecimal totalQualified, BigDecimal totalRejected) {
        if (totalRejected.signum() == 0)
            return ReceiptInspectionStatus.PASSED.getCode();
        if (totalQualified.signum() == 0)
            return ReceiptInspectionStatus.FAILED.getCode();
        return ReceiptInspectionStatus.PARTIAL.getCode();
    }
    private InventoryTransaction buildTransaction(PurchaseInbound inbound, PurchaseInboundLine line,
            BigDecimal quantity, String type, String operator) {
