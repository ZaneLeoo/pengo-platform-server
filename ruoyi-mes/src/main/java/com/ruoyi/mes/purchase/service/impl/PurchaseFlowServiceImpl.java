...
public class PurchaseFlowServiceImpl implements IPurchaseFlowService {
    ...
    public void approveReceipt(Long id, String operator) {
        ...
        for (PurchaseReceiptLine line : lines) {
            ...
            if (flowMapper.increaseOrderReceived(
                            line.getSourceOrderLineId(), line.getReceivedQuantity())
                    != 1) {
                throw new ServiceException(
                        "物料 " + line.getMaterialCode() + " 的到货数量超过订单剩余数量，或来源订单未审核");
            }
        }
    ...
    public void unapproveReceipt(Long id, String operator) {
        ...
        for (PurchaseReceiptLine line : receiptMapper.selectLines(id)) {
            if (flowMapper.increaseOrderReceived(
                            line.getSourceOrderLineId(), line.getReceivedQuantity().negate())
                    != 1) throw new ServiceException("订单到货数量反写失败");
        }
        ...
        for (PurchaseReceiptLine line : lines) {
            ...
            if (result == null
                    || result.getQualifiedQuantity()
                                    .add(result.getRejectedQuantity())
                                    .compareTo(line.getReceivedQuantity())
                            != 0) {
            ...
            if (flowMapper.updateReceiptInspection(line) != 1
                    || flowMapper.increaseOrderQualified(
                                    line.getSourceOrderLineId(), result.getQualifiedQuantity())
                            != 1) {
    ...
    public void uninspectReceipt(Long id, String operator) {
        ...
        for (PurchaseReceiptLine line : receiptMapper.selectLines(id)) {
            if (flowMapper.increaseOrderQualified(
                            line.getSourceOrderLineId(), line.getQualifiedQuantity().negate())
                    != 1) throw new ServiceException("订单合格数量反写失败");
        }
        flowMapper.resetReceiptInspection(id);
    ...
    public void approveInbound(Long id, String operator) {
        ...
        for (PurchaseInboundLine line : lines) {
            ...
            if (flowMapper.increaseReceiptInbound(
                                    line.getSourceReceiptLineId(), line.getInboundQuantity())
                            != 1
                    || flowMapper.increaseOrderInbound(
                                    line.getSourceOrderLineId(), line.getInboundQuantity())
                            != 1) {
                throw new ServiceException(
                        "物料 " + line.getMaterialCode() + " 的可入库数量不足，或来源送货单未完成质检");
            }
            InventoryTransaction transaction =
                    buildTransaction(
                            ...
                            InventoryTransactionType.INBOUND.getCode(),
                            operator);
    ...
    public void unapproveInbound(Long id, String operator) {
        ...
        for (PurchaseInboundLine line : inboundMapper.selectLines(id)) {
            if (flowMapper.increaseReceiptInbound(
                                    line.getSourceReceiptLineId(),
                                    line.getInboundQuantity().negate())
                            != 1
                    || flowMapper.increaseOrderInbound(
                                    line.getSourceOrderLineId(), line.getInboundQuantity().negate())
                            != 1) {
            ...
            InventoryTransaction transaction =
                    buildTransaction(
                            ...
                            line,
                            line.getInboundQuantity(),
                            InventoryTransactionType.INBOUND_REVERSE.getCode(),
                            operator);
    ...
    /** 查询可参照生成到货单的订单明细。 */
    @Override
    public List<ReceiptReferenceLine> selectReceiptReferenceLines(
    ...
    /** 查询可参照生成入库单的已质检到货明细。 */
    @Override
    public List<InboundReferenceLine> selectInboundReferenceLines(
    ...
    private PurchaseReceipt requiredReceipt(Long id) {
        ...
        if (receipt == null) throw new ServiceException("送货单不存在");
        return receipt;
    }

    private PurchaseInbound requiredInbound(Long id) {
        PurchaseInbound inbound = inboundMapper.selectById(id);
        if (inbound == null) throw new ServiceException("入库单不存在");
        return inbound;
    }
