...
public interface BomItemMapper {
    ...
    BomItem selectBomItemByLineNo(
            @Param("bomVersionId") Long bomVersionId, @Param("lineNo") Integer lineNo);
    ...
    List<BomItem> selectBomItemChildren(
            @Param("bomVersionId") Long bomVersionId,
            @Param("parentItemCode") String parentItemCode);

    /** 跨BOM懒加载：按子件编码和版本查子件，versionId为空则取默认版本 */
    List<BomItem> selectBomItemByComponentCode(
            @Param("componentItemCode") String componentItemCode,
            @Param("bomVersionId") Long bomVersionId);
}
