        ...
        for (BomOcrItem item : items) {
            if (item.getLineNo() == null) {
                ...
                if (!lineNos.add(item.getLineNo())) {
                    issues.add(
                            error("DUPLICATE_LINE_NO", "行号 " + item.getLineNo() + " 重复", item.getLineNo(), "lineNo"));
                }
            ...
            if (item.getQuantity() == null || BigDecimal.ZERO.compareTo(item.getQuantity()) >= 0) {
                issues.add(
                        error("INVALID_QUANTITY", "行 " + label(item) + " 数量为空或小于等于 0", item.getLineNo(), "quantity"));
            }
            if (StringUtils.isBlank(item.getComponentCodeCandidate()) && StringUtils.isBlank(item.getDrawingNo())) {
                issues.add(warn("MISSING_COMPONENT_CODE", "行 " + label(item) + " 缺少子件编码候选和图号", item.getLineNo(),
                        "componentCodeCandidate"));
            }
        }
