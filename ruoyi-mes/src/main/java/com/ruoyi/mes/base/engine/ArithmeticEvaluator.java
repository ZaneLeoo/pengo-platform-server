...
public class ArithmeticEvaluator {
    ...
    static class Token {
        Kind kind;
        String value;
        Token(Kind kind, String value) { this.kind = kind; this.value = value; }
    ...
    static class TokenStream {
        List<Token> tokens;
        int pos;
        TokenStream(List<Token> tokens) { this.tokens = tokens; this.pos = 0; }
        Token peek() { return tokens.get(pos); }
        Token next() { return tokens.get(pos++); }
        void expect(String op) {
            if (!peek().is(op)) throw new IllegalArgumentException("期望 '" + op + "', 实际: " + peek().value);
            next();
        }
        void expect(Kind kind) {
            if (peek().kind != kind) throw new IllegalArgumentException("期望数字, 实际: " + peek().value);
        }
    }
}
