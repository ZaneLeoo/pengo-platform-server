...
public class PurchaseApprovalAutomationService {
    ...
    public PurchaseApprovalAutomationService(
    ...
    public PurchaseApprovalDraft prepareInbound(String documentCode) {
        ...
        List<PurchaseInbound> matched =
                inboundMapper.selectList(query).stream()
                        .filter(item -> documentCode.equals(item.getInboundCode()))
                        .toList();
        if (matched.size() != 1)
            throw new ServiceException(
                    matched.isEmpty() ? "采购入库单不存在：" + documentCode : "采购入库单编号不唯一：" + documentCode);
    ...
    public AutomationDocumentResult approveOrder(
            DocumentActionRequest request, Long userId, String username) {
        return execute(
                request,
                userId,
                "PURCHASE_ORDER_APPROVE",
                ...
                () -> {
                    ...
                    PurchaseOrder order =
                            orderMapper.selectPurchaseOrderById(request.getDocumentId());
                    return result(order.getId(), order.getOrderCode(), order.getStatus());
                });
    }

    @Transactional(rollbackFor = Exception.class)
    public AutomationDocumentResult approveReceipt(
            DocumentActionRequest request, Long userId, String username) {
        return execute(
                request,
                userId,
    ...
    @Transactional(rollbackFor = Exception.class)
    public AutomationDocumentResult approveInbound(
            DocumentActionRequest request, Long userId, String username) {
        return execute(
                request,
                userId,
    ...
    private AutomationDocumentResult execute(
            DocumentActionRequest request, Long userId, String type, String prefix, Action action) {
        if (request == null
    ...
    private PurchaseApprovalDraft base(
            String actionType,
            Long id,
            String code,
            String status,
    ...
    private PurchaseApprovalLine receiptLine(PurchaseReceiptLine source) {
        PurchaseApprovalLine line =
                common(
                        ...
                        source.getSpec(),
                        source.getUnit(),
                        source.getReceivedQuantity());
        line.setSourceOrderCode(source.getSourceOrderCode());
        line.setWarehouseCode(source.getWarehouseCode());
        line.setWarehouseName(source.getWarehouseName());
        line.setLocationCode(source.getLocationCode());
        line.setLocationName(source.getLocationName());
    ...
    private PurchaseApprovalLine common(
            Integer lineNo,
            String materialCode,
            String materialName,
            String spec,
            String unit,
            BigDecimal quantity) {
        PurchaseApprovalLine line = new PurchaseApprovalLine();
    ...
    private PurchaseReceipt uniqueReceipt(String code, List<PurchaseReceipt> values) {
        List<PurchaseReceipt> matched =
                values.stream().filter(item -> code.equals(item.getReceiptCode())).toList();
        if (matched.size() != 1)
            throw new ServiceException(
                    matched.isEmpty() ? "采购到货单不存在：" + code : "采购到货单编号不唯一：" + code);
