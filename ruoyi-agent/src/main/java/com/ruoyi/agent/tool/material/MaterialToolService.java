    ...
    public AgentToolResult<List<MaterialToolItem>> query(MaterialToolQuery request) {
        ...
        List<Material> matched =
                materialService.selectMaterialListForAgent(
                        query.getKeyword(), query.getCategoryId(), query.getMaterialType(), status);
        boolean truncated = matched.size() > MAX_RESULT_SIZE;
        List<MaterialToolItem> data =
        ...
        if (data.isEmpty()) {
            return AgentToolResults.noResult(
                    MaterialToolResultCode.MATERIAL_NOT_FOUND, "当前条件没有查询到物料", data, meta);
        }
        return AgentToolResults.success(
                MaterialToolResultCode.MATERIAL_QUERY_SUCCESS,
                "查询到 " + data.size() + " 条物料",
                data,
                meta);
    }
