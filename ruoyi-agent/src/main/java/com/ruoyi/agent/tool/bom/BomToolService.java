...
public class BomToolService {
    ...
    public AgentToolResult<List<BomToolItem>> query(BomToolQuery request) {
        ...
        List<BomMaster> matched =
                bomMasterService.selectBomMasterListForAgent(
                        query.getKeyword(), query.getBomType(), status);
        boolean truncated = matched.size() > MAX_RESULT_SIZE;
        List<BomToolItem> data =
                matched.stream().limit(MAX_RESULT_SIZE).map(this::toBomItem).toList();
        AgentToolMeta meta = AgentToolMeta.collection(matched.size(), truncated);
        if (data.isEmpty()) {
            return AgentToolResults.noResult(
                    BomToolResultCode.BOM_NOT_FOUND, "当前条件没有查询到BOM", data, meta);
        }
        return AgentToolResults.success(
                BomToolResultCode.BOM_QUERY_SUCCESS, "查询到 " + data.size() + " 个BOM", data, meta);
    }
    ...
    public AgentToolResult<BomStructureToolResult> structure(BomStructureQuery request) {
        ...
        if (matched.isEmpty()) {
            return AgentToolResults.noResult(
                    BomToolResultCode.BOM_NOT_FOUND,
                    "没有找到指定的BOM",
                    null,
                    AgentToolMeta.collection(0, false));
        }
        if (matched.size() > 1) {
            return AgentToolResults.ambiguous(
                    BomToolResultCode.BOM_AMBIGUOUS,
                    "查询条件匹配到多个BOM，请明确选择",
                    List.of(bomIssue(matched)),
                    null);
        }
        ...
        if (selection.version == null) {
            return selection.ambiguous
                    ? AgentToolResults.ambiguous(
                            BomToolResultCode.BOM_VERSION_AMBIGUOUS,
                            "该BOM存在多个版本，请明确选择版本",
                            List.of(versionIssue(versions)),
                            null)
                    : AgentToolResults.noResult(
                            BomToolResultCode.BOM_VERSION_NOT_FOUND,
                            "没有找到指定的BOM版本",
                            null,
                            AgentToolMeta.collection(0, false));
        }
        List<BomComponentToolItem> components = components(selection.version.getId());
        BomStructureToolResult data =
                new BomStructureToolResult(
                        master.getId(),
                        master.getBomCode(),
                        master.getParentItemCode(),
                        master.getParentItemName(),
                        master.getParentItemSpec(),
                        master.getParentItemUnit(),
                        master.getBomType(),
                        master.getStatus(),
                        toVersionItem(selection.version),
                        components);
        return AgentToolResults.success(
                BomToolResultCode.BOM_STRUCTURE_SUCCESS,
                "查询到BOM版本 "
                        + selection.version.getVersionCode()
                        + " 的 "
                        + components.size()
                        + " 个子件",
                data,
                AgentToolMeta.collection(components.size(), false));
    }
    ...
    private List<BomMaster> findMasters(BomStructureQuery query) {
        ...
        String keyword =
                hasText(query.getBomCode()) ? query.getBomCode() : query.getParentItemCode();
        if (!hasText(keyword)) {
            return List.of();
        }
        ...
        if (hasText(query.getBomCode())) {
            return matched.stream()
                    .filter(item -> query.getBomCode().equalsIgnoreCase(item.getBomCode()))
                    .toList();
        }
        return matched.stream()
                .filter(
                        item ->
                                query.getParentItemCode()
                                        .equalsIgnoreCase(item.getParentItemCode()))
                .toList();
    }
    ...
    private VersionSelection selectVersion(BomStructureQuery query, List<BomVersion> versions) {
        if (query.getVersionId() != null) {
            return new VersionSelection(
                    versions.stream()
                            .filter(item -> Objects.equals(query.getVersionId(), item.getId()))
                            .findFirst()
                            .orElse(null),
                    false);
        }
        if (hasText(query.getVersionCode())) {
            return new VersionSelection(
                    versions.stream()
                            .filter(
                                    item ->
                                            query.getVersionCode()
                                                    .equalsIgnoreCase(item.getVersionCode()))
                            .findFirst()
                            .orElse(null),
                    false);
        }
        BomVersion defaultVersion =
                versions.stream()
                        .filter(item -> Integer.valueOf(1).equals(item.getDefaultFlag()))
                        .findFirst()
                        .orElse(null);
    ...
    private BomToolItem toBomItem(BomMaster master) {
        return new BomToolItem(
                master.getId(),
                master.getBomCode(),
                master.getParentItemId(),
                master.getParentItemCode(),
                master.getParentItemName(),
                master.getParentItemSpec(),
                master.getParentItemUnit(),
    ...
    private BomVersionToolItem toVersionItem(BomVersion version) {
        return new BomVersionToolItem(
                version.getId(),
                version.getVersionCode(),
                version.getVersionName(),
                version.getVersionDesc(),
                version.getBaseQty(),
                version.getUsageType(),
                version.getEffectiveDate(),
                version.getExpireDate(),
                version.getStatus(),
                version.getApproveStatus(),
                Integer.valueOf(1).equals(version.getDefaultFlag()));
    }

    private BomComponentToolItem toComponent(BomItem item) {
        return new BomComponentToolItem(
                item.getId(),
                item.getLineNo(),
                item.getComponentItemId(),
                item.getComponentItemCode(),
                item.getComponentItemName(),
                item.getComponentItemSpec(),
                item.getComponentItemUnit(),
                item.getComponentAttribute(),
                item.getComponentQty(),
                item.getFixedLossQty(),
                item.getChangeLossRate(),
                item.getSupplyType(),
                Integer.valueOf(1).equals(item.getIsVirtual()),
                Integer.valueOf(1).equals(item.getMrpExpandFlag()),
                Boolean.TRUE.equals(item.getHasChildBom()),
                item.getComponentBomVersionId());
    }
    ...
    private AgentToolIssue bomIssue(List<BomMaster> masters) {
        List<AgentToolCandidate> candidates =
                masters.stream()
                        .map(
                                item ->
                                        new AgentToolCandidate(
                                                String.valueOf(item.getId()),
                                                item.getBomCode(),
                                                item.getParentItemCode()
                                                        + " / "
                                                        + item.getParentItemName()))
                        .toList();
        return new AgentToolIssue(
                "BOM_SELECTION_REQUIRED", "bomId", "请选择一个BOM", "BOM主键ID", candidates);
    }
    ...
    private AgentToolIssue versionIssue(List<BomVersion> versions) {
        List<AgentToolCandidate> candidates =
                versions.stream()
                        ...
                        .toList();
        return new AgentToolIssue(
                "BOM_VERSION_SELECTION_REQUIRED",
                "versionId",
                "请选择一个BOM版本",
                "BOM版本主键ID",
                candidates);
