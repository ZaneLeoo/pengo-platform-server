...
public class PurchaseFlowToolService {
    ...
    public AgentToolResult<List<ReceiptReferenceLine>> receiptCandidates(
        ...
        List<ReceiptReferenceLine> data =
                automationService.receiptCandidates(orderCode, supplierName, materialCode);
        AgentToolMeta meta = AgentToolMeta.collection(data.size(), false);
        return data.isEmpty()
                ? AgentToolResults.noResult(
                        PurchaseFlowToolResultCode.RECEIPT_CANDIDATES_FOUND,
                        "没有查询到可到货的采购订单明细",
                        data,
                        meta)
                : AgentToolResults.success(
                        PurchaseFlowToolResultCode.RECEIPT_CANDIDATES_FOUND,
                        "已查询到 " + data.size() + " 条可到货采购订单明细",
                        data,
                        meta);
    }
    ...
    public AgentToolResult<ReceiptDraft> prepareReceipt(ReceiptDraftRequest request) {
        try {
            ...
            return AgentToolResults.confirm(
                    PurchaseFlowToolResultCode.RECEIPT_DRAFT_READY,
                    "采购到货草稿已准备",
                    draft,
                    "由 agent-ui 展示到货确认卡片并等待用户确认。 ");
    ...
    public AgentToolResult<InspectionDraft> prepareInspection(InspectionDraftRequest request) {
        try {
            ...
            return AgentToolResults.confirm(
                    PurchaseFlowToolResultCode.INSPECTION_DRAFT_READY,
                    "采购到货质检结果已准备",
                    draft,
                    "由 agent-ui 展示质检确认卡片并等待用户确认。 ");
        } catch (ServiceException exception) {
            return rejected(exception);
        }
    }
    ...
    public AgentToolResult<List<InboundReferenceLine>> inboundCandidates(
        ...
        List<InboundReferenceLine> data =
                automationService.inboundCandidates(receiptCode, warehouseCode, materialCode);
        AgentToolMeta meta = AgentToolMeta.collection(data.size(), false);
        return data.isEmpty()
                ? AgentToolResults.noResult(
                        PurchaseFlowToolResultCode.INBOUND_CANDIDATES_FOUND,
                        "没有查询到可入库的已质检到货明细",
                        data,
                        meta)
                : AgentToolResults.success(
                        PurchaseFlowToolResultCode.INBOUND_CANDIDATES_FOUND,
                        "已查询到 " + data.size() + " 条可入库到货明细",
                        data,
                        meta);
    }
    ...
    public AgentToolResult<InboundDraft> prepareInbound(InboundDraftRequest request) {
        try {
            ...
            return AgentToolResults.confirm(
                    PurchaseFlowToolResultCode.INBOUND_DRAFT_READY,
                    "采购入库草稿已准备",
                    draft,
                    "由 agent-ui 展示入库确认卡片并等待用户确认。 ");
        } catch (ServiceException exception) {
            return rejected(exception);
        }
    }
    ...
    private <T> AgentToolResult<T> rejected(ServiceException exception) {
        AgentToolIssue issue =
                AgentToolIssue.of(
                        "BUSINESS_VALIDATION_FAILED", "", exception.getMessage(), "根据错误信息补充或修正字段");
        return AgentToolResults.rejected(
                PurchaseFlowToolResultCode.PURCHASE_FLOW_VALIDATION_FAILED,
