...
public class MaterialCategoryToolService {
    ...
    public AgentToolResult<List<MaterialCategoryToolItem>> query(
            MaterialCategoryToolQuery request) {
        MaterialCategoryToolQuery query =
                request == null ? new MaterialCategoryToolQuery() : request;
        String status = Boolean.FALSE.equals(query.getIncludeDisabled()) ? "0" : null;
        List<MaterialCategoryToolItem> data =
                categoryService
                        .selectCategoryListForAgent(query.getKeyword(), query.getParentId(), status)
                        .stream()
                        .map(this::toItem)
                        .toList();
        ...
        if (data.isEmpty()) {
            return AgentToolResults.noResult(
                    MaterialCategoryToolResultCode.MATERIAL_CATEGORY_NOT_FOUND,
                    "当前条件没有查询到物料分类",
                    data,
                    meta);
        }
        return AgentToolResults.success(
                MaterialCategoryToolResultCode.MATERIAL_CATEGORY_QUERY_SUCCESS,
                "查询到 " + data.size() + " 条物料分类",
                data,
                meta);
    }
    ...
    private MaterialCategoryToolItem toItem(MaterialCategory category) {
        ...
        return new MaterialCategoryToolItem(
