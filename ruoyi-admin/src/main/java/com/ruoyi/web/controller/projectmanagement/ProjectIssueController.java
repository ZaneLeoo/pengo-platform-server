...
public class ProjectIssueController extends BaseController {
    ...
    public AjaxResult transition(@PathVariable Long id, @Valid @RequestBody IssueTransitionRequest request) {
    ...
    public AjaxResult activities(@PathVariable Long id) {
        List<ProjectIssueActivity> activities = service.activities(id);
        return success(activities);
    }

    @PreAuthorize("@ss.hasPermi('projectManagement:issue:edit')")
    @PostMapping("/{id}/activities")
    public AjaxResult addActivity(@PathVariable Long id, @Valid @RequestBody ProjectIssueActivity activity) {
