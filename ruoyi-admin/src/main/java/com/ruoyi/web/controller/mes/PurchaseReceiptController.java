...
public class PurchaseReceiptController extends BaseController {
    ...
    @PreAuthorize("@ss.hasPermi('mes:purchaseReceipt:add')") @Log(title="采购到货",businessType=BusinessType.INSERT) @PostMapping public AjaxResult add(@RequestBody PurchaseReceipt o) { o.setCreateBy(getUsername()); mapper.insert(o); if(o.getLines()!=null)o.getLines().forEach(x->{x.setReceiptId(o.getId());mapper.insertLine(x);}); return success(o.getId()); }
    @PreAuthorize("@ss.hasPermi('mes:purchaseReceipt:edit')") @Log(title="采购到货",businessType=BusinessType.UPDATE) @PutMapping public AjaxResult edit(@RequestBody PurchaseReceipt o) { o.setUpdateBy(getUsername()); return toAjax(mapper.update(o)); }
