...
public class AgentToolExceptionHandler {
    ...
    public AgentToolResult<Void> handleValidation(MethodArgumentNotValidException exception) {
        List<AgentToolIssue> issues =
                exception.getBindingResult().getFieldErrors().stream().map(this::toIssue).toList();
        return AgentToolResults.rejected(
                CommonAgentToolResultCode.INVALID_REQUEST, "工具请求参数校验失败", issues, null);
    }
    ...
    public AgentToolResult<Void> handleUnreadableBody(HttpMessageNotReadableException exception) {
        AgentToolIssue issue =
                AgentToolIssue.of(
                        ...
                        "requestBody",
                        "请求 JSON 的字段类型或格式不正确",
                        "严格按照 OpenAPI schema 传参");
        return AgentToolResults.rejected(
                CommonAgentToolResultCode.INVALID_REQUEST, "无法解析工具请求参数", List.of(issue), null);
    }

    /** 转换字段校验错误。 */
    private AgentToolIssue toIssue(FieldError error) {
        return AgentToolIssue.of(
                "INVALID_FIELD", error.getField(), error.getDefaultMessage(), "符合 OpenAPI 中该字段的约束");
    }
}
