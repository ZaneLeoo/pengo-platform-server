...
public class BomAiImportTraceService {
    ...
    public void markImported(Long traceId, List<BomAiImportedBom> importedBoms, String operator) {
        if (traceId == null) {
            return;
        }
        ...
        if (!BomAiImportTraceStatus.RECOGNIZED.name().equals(trace.getStatus())) {
            throw new ServiceException("当前 AI 导入批次不能确认导入，状态为：" + trace.getStatus());
        }
