...
public class ProjectDeliverableServiceImpl implements IProjectDeliverableService {
    ...
    public int insert(ProjectDeliverable entity) { assertProjectAllowed(entity.getProjectId()); prepare(entity); assertTaskEditable(entity.getTaskId()); return mapper.insert(entity); }
    public int update(ProjectDeliverable entity) { assertProjectAllowed(entity.getProjectId()); prepare(entity); assertTaskEditable(entity.getTaskId()); return mapper.update(entity); }
    public int deleteByIds(Long[] ids) { for (Long id : ids) { ProjectDeliverable d = required(id); assertProjectAllowed(d.getProjectId()); assertTaskEditable(d.getTaskId()); } return mapper.deleteByIds(ids); }
    ...
    public void review(Long id, boolean approved, String comment, String username) {
        ...
        ProjectDeliverable d = required(id); assertProjectAllowed(d.getProjectId()); if (!"PENDING_APPROVAL".equals(d.getStatus())) throw new ServiceException("当前交付物不在待审批状态");
    ...
    private void assertProjectAllowed(Long projectId) { var project=projectMapper.selectProjectInfoById(projectId); if(project==null)throw new ServiceException("所属项目不存在"); if("DRAFT".equals(project.getStatus()))throw new ServiceException("项目处于申请草稿阶段，正式立项后才能维护交付物"); }
}
