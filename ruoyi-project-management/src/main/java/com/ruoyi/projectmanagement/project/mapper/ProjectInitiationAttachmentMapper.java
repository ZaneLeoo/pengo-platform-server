...
public interface ProjectInitiationAttachmentMapper {
    ...
    /** 查询当前草稿附件。 */
    List<ProjectInitiationAttachment> selectDraft(
            @Param("projectId") Long projectId, @Param("sectionCode") String sectionCode);

    /** 查询审批版本附件。 */
    List<ProjectInitiationAttachment> selectByApproval(
            @Param("approvalId") Long approvalId, @Param("sectionCode") String sectionCode);

    /** 查询项目最新审批版本附件。 */
    List<ProjectInitiationAttachment> selectLatestApproved(
            @Param("projectId") Long projectId, @Param("sectionCode") String sectionCode);
