create table if not exists pm_project_work_item (
  item_id bigint not null auto_increment, project_id bigint not null, parent_id bigint not null default 0,
  ...
  primary key(item_id), unique key uk_pm_work_item_code(item_code), key idx_pm_work_project_type(project_id,item_type), key idx_pm_work_owner(owner_id)
